...
public class SimpleBinarySchedulableMoveCommand extends SchedulableMoveCommand {
    ...
    public SimpleBinarySchedulableMoveCommand(MoveCommand cmd, SocketChannel out, int version, int marker) {
        super(cmd);
        this.out = out;
    ...
    public void reply(boolean success) throws Exception {
        ...
        try (var wrapper = BufferManager.getBuffer()) {
            ...
            buffer.putShort((short)11); // command: MOVE
    ...
    public static void handle(Executor executor, SocketChannel sc, int version, Identity user, int marker) throws Exception {
        ...
        var schedulable = new SimpleBinarySchedulableMoveCommand(cmd, sc, version, marker);
